...
class case:
    ...
    def draw_case(self,surface):
        pygame.draw.rect(surface,(0,0,0,0),self.visual,1)
...
def centrer_piece(rect, piece, plateau):
        plateau_rect = plateau.get_rect()
        offset_x = (fenetrePrincipale.get_size()[0] - plateau_rect.width) / 2
        offset_y = (fenetrePrincipale.get_size()[1] - plateau_rect.height) / 2
        return rect.left + offset_x + (rect.width - piece.get_width()) // 2, rect.top + offset_y + (rect.height - piece.get_height()) // 2
...
class pion(pièce): #ajouter mouvement diagonale lors de couleur différente
    ...
    def affichage(self):
        if self.couleur == "blanc":
            self.image = pygame.transform.scale(imgsPiecesB[0], (int(imagePlateau.get_size()[0] / 10)+10, int(imagePlateau.get_size()[0] / 10)+10))
        else:
            self.image = pygame.transform.scale(imgsPiecesN[0], (int(imagePlateau.get_size()[0] / 10)+10, int(imagePlateau.get_size()[0] / 10)+10))
        position = centrer_piece(plateau1.grille[self.y][self.x].visual, self.image, imagePlateau)
    ...
    def __init__(self):
        ...
        for y in range(1,9):
            ...
            for x in range(1,9):
                ...
                nouvelle_case.draw_case(imagePlateau)
